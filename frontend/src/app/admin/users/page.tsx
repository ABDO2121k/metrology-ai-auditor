...
export default function AdminUsersPage() {
  ...
  // No role field: the platform has a single role and the gateway assigns it.
  ...
  useEffect(() => {
    // AuthGuard has already established the session by the time this renders,
    // and apiFetch attaches the token, so there is nothing to read here.
    fetchUsers();
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      setMsg({ type: 'error', text: e?.message || 'Chargement des utilisateurs impossible' });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    setMsg(null);
    try {
      const created = await api.registerUser(regForm);
      setMsg({ type: 'success', text: `Compte utilisateur ${created.username} créé avec succès !` });
      setIsRegisterOpen(false);
      setRegForm({ username: '', email: '', password: '', full_name: '' });
      fetchUsers();
    } catch (err: any) {
      setMsg({ type: 'error', text: err?.message || 'Échec de création' });
    }
  };
  ...
  const handleAdminResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      await api.resetUserPassword(targetUser.id, newPasswordOverride);
      setMsg({
        type: 'success',
        text: `Mot de passe réinitialisé pour l'utilisateur ${targetUser.username} !`,
      });
      setIsResetOpen(false);
      setTargetUser(null);
      setNewPasswordOverride('');
    } catch (err: any) {
      setMsg({ type: 'error', text: err?.message || 'Échec de la réinitialisation' });
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="glass-panel rounded-3xl border border-slate-800 overflow-hidden">
        <div className="p-4 border-b border-slate-800 flex justify-between items-center">
          ...
          <button onClick={() => fetchUsers()} className="text-slate-400 hover:text-white p-1">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md p-4">
          <div className="glass-panel rounded-3xl p-6 max-w-md w-full border border-slate-800 space-y-4 shadow-2xl">
            ...
            <form onSubmit={handleCreateUser} className="space-y-3 text-xs">
              ...
              {/* The platform runs on one role; every account is a technician
                  with full rights, so there is nothing to choose here. */}
              <div className="p-2.5 rounded-xl bg-cyan-500/10 border border-cyan-500/30 text-[11px] text-cyan-200">
                {t('adminSingleRoleNote')}
              </div>
